...
-- ① COUNT(*)는 NULL 값을 포함한 전체 튜플(행의 개수)의 수를 센다.
...
-- 정답:
SELECT SUM(price *quantity) AS 매출액
FROM orders
WHERE price <= 1000000 AND quantity >= 2;
...
-- 정답:	
SELECT COUNT(DISTINCT region) AS 지역
FROM orders;
...
-- 정답:
SELECT DISTINCT product AS 어쩔티비
FROM orders
