...
-- 정답: id 칼럼은 고유 식별자역할이라서 음수로 표기가 불가능하고, 
-- 직원의 수가 65535명을 넘지않을 것이라 판단하여 smallint를 쓴다.
-- 메모리 절약을 위해서
...
-- 정답: 고정 소수점 숫자, 총 5자리 중 소수점 아래 2자리 포함
...
-- 정답: VARCHAR은 가변하는 문자열이고 CHAR은 고정길이 문자열이다.
...
-- 정답: DATETIME은 날짜 AND 시간 모두 저장, DATE는 날짜만 저장
...
-- 정답: ENUM은 보통 고정형 데이터를 저장할때 사용되는데, 여기서
-- 성별은 남자, 여자 둘 중 하나임으로 ENUM을 사용함
...
-- 정답: float(32bit) 단정밀도 부동 소수점, double(64bit)은 배정밀도 부동 소수점이다.
...
-- 정답: '박으로 시작하는 뒤에 두글자가 붙는 이름을 찾는다.
...
-- 정답: order_time이 9시부터 17시 사이인 행을 찾는다.
