...
-- ② 데이터를 정렬하여 출력할 때 사용한다. -- ORDER BY
-- ③ 중복된 데이터를 제거할 때 사용한다. -- DISTINCT
-- ④ 데이터를 필터링하여 출력할 때 사용한다. -- WHERE or HAVING
...
SELECT category, COUNT(*) -- 3) 카테고리안의 행의 개수를 샌다.
FROM products -- 1) products의 테이블을 참고한다.
GROUP BY category; -- 2) 카테고리를 그룹으로 묶는다.
