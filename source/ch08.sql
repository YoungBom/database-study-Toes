...
-- 오류 발생 : WHERE 절은 그룹화가 이루어지기 전, 테이블의 개별 행 하나하나에 대해 조건을 검사하기 때문에
GROUP BY ptype
HAVING AVG(amount) >= 40000; -- 집계 함수는 HAVING 절에서 조건을 거는게 맞음
...
FROM payments
GROUP BY ptype
HAVING `평균 결제 금액` >= 40000; -- 별칭에 ''를 쓰면 이것을 문자열로 인식하기 때문에 어떤 값도 출력되지않는다 이럴땐 ``(백틱을 써서 SELECT 절의 별칭을 가져다 써도 된다.)
...
-- ORDER BY 절을 사용하여 수행
-- SELECT로 조회된 데이터를 기준으로 정렬하는 직업(SELSCT가 먼저 수행됨)
...
FROM payments -- 1) playment 테이블에서
ORDER BY amount DESC; -- 3) amount 기준으로 내림차순 정렬함

-- 다중 컬럼 정렬: 결제(payments) 테이블에서 결제 유형은 오름차순, 결제 금액은 내림차순으로 정렬하려면?
SELECT 
	ptype AS '결제 유형',
    amount AS '결제 금액'
FROM payments
ORDER BY ptype ASC, amount DESC; -- ptype의 오름차순 정렬 후 그 안에서 amount의 내림차순 정렬(순서 중요)

-- 실무 Tip
-- 오름차순인 경우 ASC 키워드는 생략 가능(정렬의 기본값)
-- 실무에서는 오름차순인 경우에는 ASC 키워드를 대부분 생략하고 ORBER BY만 사용
...
-- 정답:  1.COUNT(amount) 2. ptype 3.  AVG(amount) 

SELECT 
	ptype AS '결제 유형',
	COUNT(amount) AS '결제 횟수',
	AVG(amount) AS '평균 결제 금액'
FROM payments
GROUP BY ptype
...
JOIN payments p ON o.id = p.order_id -- 3) 결제 테이블을 붙이고 
GROUP BY u.id -- 4) 사용자별 그룹화를 진행하여
...
-- 1) orders 테이블에 장바구니 정보도 함께 저장되어서 이를 구분하기 위해 JOIN을 한번 더 하고
JOIN products p ON p.id = od.product_id   
WHERE status = '배송 완료' -- 2) 배송 완료 레코드만 필터링
GROUP BY p.id, p.name -- 
ORDER BY `판매수량` DESC, p.id DESC
LIMIT 3;
