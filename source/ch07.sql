...
ON comments.user.id = users.id -- 해당 조건으로
WHERE photo_id = 1; -- 3) 1번 사진과 관련된 것만 남긴 후, 

-- 조인의 특징 7가지
-- 1) 조인 컬럼이 필요
-- 두 테이블을 연결하기 위한 공통 칼럼이 필요
...
-- 2) 조인 컬럼은 서로 자료형이 일치
-- 일치해야 조인 가능
-- 예: 숫자1과 문자열 '1'은 서로 조인 불가
-- 다만 DBMS마다 자동 형 변환이 될 수도 있음
-- 실무에서는 성능/안정성 때문에 자료형 일치가 원칙
...
-- 4) 연속적인 조인 가능
SELECT nickname, body, filename -- 4) 원하는 컬럼 조회 (닉네임, 댓글본문, 파일명)
FROM comments -- 1) 댓글 테이블에
...
-- 컬럼명이 같은 경우 어느 테이블의 것인지 명시 해야함 (그렇지 않은 경우 에러 발생)
-- 사용예: 중복 컬럼 id에 테이블명 명시
SELECT id, body, users.id, nickname
FROM comments
JOIN users ON comments.user_id = users.id
WHERE photo_id = 2;
...
SELECT
	...
	u.nickname
FROM comments AS c
JOIN sers ON comments.user_id = users.id;
-- 실무에서 테이블에 별칭을 붙일 때는 AS를 보통 생략 함
...
-- 예: 홍팍이가 올린 모든 사진의 좋아요 개수는?
SELECT u.nickname, COUNT(l.id) AS 좋아요
FROM users u
