...
-- 2단계: 그 평균 가격보다 비싼 상품을 찾는다. => WHERE 절 사용
...
FROM payments;
 ...
 SELECT 
	order_id  AS '주문자 정보',
	SUM(count) AS '평균 상품 개수' -- 집계 함수 결과에 별칭 필수
 FROM order_details od
 GROUP BY order_id -- 서브쿼리로 사용
 ...
 */
 ...
 SELECT *
 FROM products
 WHERE price < ANY(2900,1300); -- 하드 코딩방식
 ...
 -- 3. ALL 연산자
 -- 지정된 집합의 모든 요소와 비교 연산을 수행하여 모두를 만족하는 대상을 찾음ALTER
...
WHERE EXISTS (
	...
	FROM orders ON
	WHERE o.users_id = u.id -- 이렇게 서브쿼리가 메인쿼리의 특정 값을 참조하는 쿼리를 '상관 서브쿼리'라고 함
);
...
WHERE id IN (
	SELECT DISTINCT user_id
	FROM orders -- 서브쿼리의 결과값이 많아질수록 성능이 저하
	);

-- 또 다른 방법2(권장)
SELECT DISTINCT u.* -- 사용자의 모든 컬럼을 보여준다.
FROM users u 
JOIN orders o ON u.id = o.user_id;
