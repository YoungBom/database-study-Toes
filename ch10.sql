...
-- 주문은 특정 사용자를 참조할 수도 있고, 또는 참조하지 않을 수도 있다. 즉, 비회원인 경우에도 주문 가능(0~1)
...
-- 주문 -> 상품은 필수적 일대다 관계, 상품 -> 주문 선택적 일대다 관계
...
-- 기술팀이 전문가 관점에서 데이터베이스의 성능과 효율성을 고려해 테이블을 설계

-- 1) 엔티티는 테이블로, 속성은 컬럼으로 변경
-- 각 테이블의 기본키는 id 또는 테이블명_id로, 외래키는 XXX_id로 명명해 일관성을 높임
-- 테이블명_id는 조인할 때 가독성이 좋고 어떤 테이블의 PK인지 명확하게 알 수 있음

-- 2) 컬럼의 자료형 정의(DBMS에 맞게)
-- 예: MySQL에서 지원하는 자료형 중 해당 컬럼을 가장 잘 저장할 수 있는 것으로 표시

-- 3) 고유키(+제약 조건) 및 인덱스 지정
-- users 테이블의 email과 products 테이블의 name을 고유키로 지정
-- 고유키는 UK로 표시
-- products 테이블의 name을 인덱스 처리
-- 상품명으로 검색 및 정렬할 때 성능을 높임

-- 고유키(Unique Key, UK)란 테이블 내에서 값이 고유해야 하는 특정 컬럼 또는 컬럼의 조합을 말함
-- 고유 키로 설정한 컬럼에 중복 값이 입력될 경우 에러가 발생하므로 중복 입력을 예방하려면 고유키로 지정
